...
public class PackageUtil {
	...
	public static List<Class<?>> getClass(String packageName, boolean recursive) {
		...
		try {
			...
			Enumeration<URL> iterator = Thread.currentThread().getContextClassLoader().getResources(packageName.replace(ExcelConstant.DOT_CHAR, File.separatorChar));
	...
	public static List<Class<?>> getClassInFile(Path path, String packageName, boolean recursive) {
		...
		if (Files.isDirectory(path)) {
			...
			try {
				...
				int lastDotIndex = pathStr.lastIndexOf(ExcelConstant.DOT_CHAR);
				//Class.forName只允许使用用'.'分隔的类名的形式
				String className = pathStr.replace(File.separatorChar, ExcelConstant.DOT_CHAR);
				//获取包名的起始位置
		...
		while (iterator.hasMoreElements()) {
			...
			if (!jarEntry.isDirectory()) {
				...
				if(lastDotClassIndex != -1) {
					int lastSlashIndex = name.lastIndexOf(ExcelConstant.SLASH_CHAR);
					name = name.replace(ExcelConstant.SLASH_CHAR, ExcelConstant.DOT_CHAR);
